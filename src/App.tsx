...
function Layout() {
  const links = [
    ...
    },
    {
      to: '/account',
      label: 'Account',
      icon: <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>,
    },
  ];
        ...
        <nav className="flex-1 p-3 space-y-0.5">
          {links.map((l) => (
            <NavLink
              ...
              to={l.to}
              className={({ isActive }) =>
                `flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium uppercase tracking-widest transition-colors ${isActive ? 'bg-[rgba(94,162,182,0.15)] text-white' : 'text-[rgba(255,255,255,0.5)] hover:bg-[rgba(255,255,255,0.05)] hover:text-[rgba(255,255,255,0.87)]'}`
              }
            >
              {l.icon}
              {l.label}
